...
export async function getS3Url(s3Locator: AwsS3LocatorProperties, s3OrRegion: S3 | string, force = true): Promise<string> {
    ...
    if (typeof s3OrRegion !== "string") {
        const s3 = s3OrRegion;
        const data = await s3.getBucketLocation({ Bucket: s3Locator.awsS3Bucket }).promise();
    ...
    // build the url
    const url = `https://${s3SubDomain}.amazonaws.com/${s3Locator.awsS3Bucket}/${getS3RelativePath(s3Locator)}`;
